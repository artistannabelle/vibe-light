...
## 1.3.0 — 2026-06-07（GitHub 公开发布）

连续任务黄灯更准，节约时间展示更简洁。升级后请重启 Cursor。

### 相对 1.2.0
- **连续任务黄灯**：Grep、Shell 等多步执行时保持黄灯，步骤间不再误变绿
- **节约时间**：主窗口只显示总数（如「4 分钟」）及 ⓘ 说明；预览条「节时 4m」；无数据时「0 秒」；隐藏分组明细

## 1.2.3 — 2026-06-07

### 节约时间展示
- 恢复主窗口「今天节约了多少时间」总数（长文案，如「4 分钟」）及 ⓘ 说明
- 恢复预览条「节时 4m」短标签
- 仍隐藏分组 1/2/3 明细；无数据时显示「0 秒」

## 1.2.2 — 2026-06-07

### 连续任务黄灯
- 新增 `preToolUse` Hook：任意工具（Grep、Shell 等）执行前刷新 `agent_activity.json` 并变黄灯
- 采集器读取 activity，在确认本轮结束前保持黄灯；`stop` 不再清除 activity，避免连续步骤中间误变绿

## 1.2.1 — 2026-06-07

### UI
- 主窗口隐藏「今天节约了多少时间」及分组 1/2/3 明细（后台仍统计，不对用户展示）
- 预览条隐藏「节时」芯片

## 1.2.0 — 2026-06-07（GitHub 公开发布）
